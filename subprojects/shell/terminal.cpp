...
namespace terminal {
    ...
    void init() {
        // Framebuffer
        if (!sys::open("/dev/framebuffer", sys::Mode::ReadWrite, sys::FileFlags::CloseOnExecute, fb)) {
            sys::exit(1);
        }
        ...
        uint32_t timer;
        if (!sys::open("/dev/timer", sys::Mode::Read, sys::FileFlags::CloseOnExecute, timer)) {
            sys::exit(1);
        }
    ...
    uint64_t read(char* buffer, const uint64_t length) {
        uint32_t kb;
        if (!sys::open("/dev/keyboard", sys::Mode::Read, sys::FileFlags::CloseOnExecute, kb)) return 0;
