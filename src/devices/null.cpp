...
namespace cosmos::devices::null {
    static uint64_t seek(const stl::Rc<vfs::File>& file, const vfs::SeekType type, const int64_t offset) {
    ...
    static uint64_t write([[maybe_unused]] const stl::Rc<vfs::File>& file, [[maybe_unused]] const void* buffer, const uint64_t length) {
        return length;
    }

    static uint64_t ioctl([[maybe_unused]] const stl::Rc<vfs::File>& file, [[maybe_unused]] uint64_t op, [[maybe_unused]] uint64_t arg) {
